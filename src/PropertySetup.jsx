...
// Each zone is traced on its OWN photo — same image used in the planner
const ZONES = [
  {
    key: 'bedA',
...
export default function PropertySetup({ onDone }) {
  ...
  const [step, setStep]       = useState(0)          // 0 = bedA, 1 = bedB
  ...
  function handleClick(e) {
    ...
    if (pts.length >= 3) {
      const first = pts[0]
      if (Math.hypot(pt.x - first.x, pt.y - first.y) < 0.03) {
        finishZone(pts)
        return
      }
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', background: '#f2efe8' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #ddd8cc', padding: '12px 20px', flexShrink: 0 }}>
        <div style={{ fontSize: 16, fontWeight: 600, color: '#2a3a18', marginBottom: 3 }}>
          Bed Setup — {zone.label}
        </div>
        <div style={{ fontSize: 11, color: '#7a8a68' }}>
          {zone.instruction} Double-click or click the first ● to close the outline.
        </div>
      </div>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #ddd8cc', padding: '8px 16px', display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0, flexWrap: 'wrap' }}>
        {ZONES.map((z, i) => (
          <button key={z.key}
            style={{ ...btnStyle(step === i), borderColor: step === i ? (z.key === 'bedB' ? '#1060c0' : '#4a9020') : '#c8c0a8', background: step === i ? (z.key === 'bedB' ? '#dde8ff' : '#d8eeb8') : '#fff', color: step === i ? (z.key === 'bedB' ? '#1a2a6a' : '#1a4008') : '#3a4a28' }}
            onClick={() => { setStep(i); setDrawing(false); setPts([]) }}
          >
            {zones[z.key] ? `✓ ${z.label}` : z.label}
          </button>
        ))}
        ...
        {!drawing ? (
          <button style={btnStyle(false)} onClick={startDraw}>
            ✏️ {savedPoly ? 'Retrace this bed' : 'Trace this bed'}
          </button>
        ) : (
          <button style={btnStyle(true, true)} onClick={() => { setDrawing(false); setPts([]) }}>
            ✕ Cancel
          </button>
        )}

        {savedPoly && !drawing && (
          <button style={{ ...btnStyle(false), color: '#c05010', borderColor: '#e0a888' }} onClick={clearZone}>
            Clear outline
          </button>
        )}
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          {!drawing && savedPoly && step < ZONES.length - 1 && (
            <button style={{ ...btnStyle(true), background: '#1060c0', color: '#fff', borderColor: '#0840a0' }}
              onClick={() => { setStep(s => s + 1); setPts([]) }}>
              Next → {ZONES[step + 1].label}
            </button>
          )}
          {allDone && !drawing && (
        ...
        >
          ...
          {size.w > 0 && (
            <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none' }}
              ...
              {/* Saved polygon for this bed */}
              {savedPoly && (
                <polygon
                  points={savedPoly.map(toSVG).join(' ')}
                  fill={zone.color}
                  stroke={zone.stroke}
                  strokeWidth="2.5"
                  strokeDasharray="6,3"
